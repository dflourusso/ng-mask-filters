...
gulp.task 'build', ->
  ...
  js =
    gulp.src 'src/**/*.coffee'
    .pipe $.plumber()
    .pipe $.ngClassify(appName: 'ng-mask-filters')
    .pipe $.concat('ng-mask-filters.js')
    .pipe $.coffee(bare: no)
  ...
  es.merge(lib, js)
    ...
    .pipe gulp.dest 'dist'
    .pipe($.uglify())
    .pipe($.concat('ng-mask-filters.min.js'))
    .pipe(gulp.dest('dist'))

gulp.task 'examples', ->
  gulp.src 'examples/**/*.coffee'
  .pipe $.plumber()
  .pipe $.ngClassify(appName: 'ng-mask-filters-examples')
  .pipe $.coffee(bare: no)
